...
export function useTrackingData(userId: string | null) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: sh }, { data: ds }, { data: ff }, { data: ph }, { data: dh }, { data: bs }, { data: schH }] = await Promise.all([
      ...
      setSchoolHolidays((schH ?? []) as SchoolHoliday[]);
  ...
  return { shifts, sessions, fillups, holidays, declared, balanceSettings, schoolHolidays, activeShift, activeSession, loading, refresh };
}
